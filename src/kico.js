...
(function (global, setting) {
    ...
    KStyle.fn = KStyle.prototype = {
        construtor: KStyle, init: function (a, b) {
    ...
    var notice = {wrap: KStyle.create("notice"), list: []};
    ...
    image_box.wrap = KStyle.create("div", {
        class: "ks-image",
        child: [image_box.prev, image_box.img, image_box.next, image_box.ball]
    });
            ...
            if (request.readyState === 4) {
                if (request.status === 200 || request.status === 304) {
                    if (prop.type) {
                        switch (prop.type) {
                            case "json":
                                prop.success(JSON.parse(request.response));
                                break;
                        }
                    } else {
        ...
        el.forEach(function (t) {
            t.onclick = function (e) {
                ...
                if (c === l) {
                    ...
                    "scrollBehavior" in document.documentElement.style ? global.scrollTo({
                        top: top,
                        left: 0,
                        behavior: "smooth"
